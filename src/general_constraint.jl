...
function constraint_indices(general::GeneralConstraint{T}; shift=0) where T
    return collect(shift .+ (1:general.num_constraint))
end

function jacobian_indices(general::GeneralConstraint{T}; shift=0) where T
    return collect(shift .+ (1:general.num_jacobian))
end
