...
function TrajectoryOptimizationData(
    ...
    return TrajectoryOptimizationData(
        states,
        actions,
        parameters,
        objective,
...
function indices(
    ...
    # Jacobians
    dynamics_jacobians = jacobian_indices(dynamics; shift=0)
    stage_jacobians = jacobian_indices(constraints; shift=num_jacobian(dynamics))
    general_jacobian = jacobian_indices(
        general;
        shift=(num_jacobian(dynamics) + num_jacobian(constraints))
    )

    # Constraints
    dynamics_constraints = constraint_indices(dynamics; shift=0)
    stage_constraints = constraint_indices(constraints; shift=num_constraint(dynamics))
    general_constraint = constraint_indices(
        general;
...
function primal_bounds(
    bounds::Bounds{T},
    num_variables::Int,
    state_indices::Vector{Vector{Int}},
    action_indices::Vector{Vector{Int}}
) where T
    lower, upper = fill(-Inf, num_variables), fill(Inf, num_variables)
...
function NLPData(
    trajopt::TrajectoryOptimizationData;
    ...
    sparsity_dynamics = sparsity_jacobian(
        trajopt.dynamics,
    ...
    sparsity_constraint = sparsity_jacobian(
        trajopt.constraints,
        trajopt.state_dimensions,
        trajopt.action_dimensions,
    ...
    sparsity_general = sparsity_jacobian(
        general_constraint,
        total_variables;
        row_shift=(num_dynamics + num_stage)
    )

    jacobian_sparsity = collect(
        [sparsity_dynamics..., sparsity_constraint..., sparsity_general...]
    )

    # Hessian of Lagrangian sparsity
    sparsity_objective_hessians = sparsity_hessian(
        trajopt.objective,
        trajopt.state_dimensions,
        trajopt.action_dimensions
    )

    sparsity_dynamics_hessians = sparsity_hessian(
        trajopt.dynamics,
        trajopt.state_dimensions,
        trajopt.action_dimensions
    )

    sparsity_constraint_hessian = sparsity_hessian(
        trajopt.constraints,
        trajopt.state_dimensions,
        trajopt.action_dimensions
    )

    sparsity_general_hessian = sparsity_hessian(
        general_constraint,
        total_variables
    )

    hessian_lagrangian_sparsity = [
        sparsity_objective_hessians...,
        sparsity_dynamics_hessians...,
        sparsity_constraint_hessian...,
        sparsity_general_hessian...
    ]

    if isempty(hessian_lagrangian_sparsity)
        hessian_lagrangian_sparsity = Tuple{Int,Int}[]
    end
    ...
    primal_lower, primal_upper = primal_bounds(
        trajopt.bounds, total_variables, idx.states, idx.actions
    )

    # nonlinear constraint bounds
    constraint_lower, constraint_upper = constraint_bounds(
        trajopt.constraints, general_constraint, num_dynamics, num_stage, idx
    )
...
function SolverData(nlp::NLPData; options=Options())
    ...
    variables = MOI.add_variables(optimizer, nlp.num_variables)

    for i = 1:nlp.num_variables
        MOI.add_constraint(
            optimizer,
            variables[i],
            MOI.LessThan(nlp.variable_bounds[2][i])
        )
        MOI.add_constraint(
            optimizer,
            variables[i],
            MOI.GreaterThan(nlp.variable_bounds[1][i])
        )
    end
...
function duals!(
    ...
    duals_general::Vector{T},
    duals,
    dynamics_indices::Vector{Vector{Int}},
    constraint_indices::Vector{Vector{Int}},
    general_indices::Vector{Int}
) where T
