...
function Constraint(
    ...
    evaluate_built = Symbolics.build_function(evaluate, x, u, w)
    # if isa(evaluate_built, Tuple)
    #     evaluate_func! = eval(evaluate_built[2])
    # else
    #     evaluate_func = eval(evaluate_built)
...
function num_constraint(constraints::Constraints{T}) where T
    return sum([con.num_constraint for con in constraints])
end

function num_jacobian(constraints::Constraints{T}) where T
    return sum([con.num_jacobian for con in constraints])
end

# function num_hessian(constraints::Constraints{T}) where T
#     return sum([con.num_hessian for con in constraints])
# end

function constraint_indices(
    constraints::Constraints{T};
    shift=0
) where T
