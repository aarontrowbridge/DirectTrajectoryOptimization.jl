...
function Dynamics(
    ...
    evaluate = f(y, x, u, w)
    built_evaluate = Symbolics.build_function(evaluate, y, x, u, w)
    # if isa(built_evaluate, Tuple)
    #     evaluate_func! = eval(built_evaluate[2])
    # else
    #     evaluate_func = eval(Symbolics.build_function(evaluate, y, x, u, w))
    #     evaluate_func! = (out, args...) -> out .= evaluate_func(args...)
    # end
    evaluate_func! = eval(built_evaluate[2])
...
function Dynamics(
    ...
    jacobian_func = (J, y, x, u, w) -> constraint_jacobian(
        reshape(view(J, :), num_next_state, num_variables), y, x, u, w
    )
...
function num_state_action_next_state(constraints::Vector{Dynamics{T}}) where T
    num_next_state = constraints[end].num_next_state
    return sum([con.num_state + con.num_action for con in constraints]) + num_next_state
end

function num_constraint(constraints::Vector{Dynamics{T}}) where T
    return sum([con.num_next_state for con in constraints])
end

function num_jacobian(constraints::Vector{Dynamics{T}}) where T
    return sum([con.num_jacobian for con in constraints])
end

# function num_hessian(constraints::Vector{Dynamics{T}}) where T
#     return sum([con.num_hessian for con in constraints])
# end

function constraint_indices(constraints::Vector{Dynamics{T}}; shift=0) where T
    return [collect(
        shift + (t > 1 ?
            sum(
                [constraints[s].num_next_state for s = 1:(t-1)]
            ) : 0
        ) .+ (1:constraints[t].num_next_state)
    ) for t = 1:length(constraints)]
end

function jacobian_indices(constraints::Vector{Dynamics{T}}; shift=0) where T
    return [collect(
        shift + (t > 1 ?
            sum(
                [constraints[s].num_jacobian for s = 1:(t-1)]
            ) : 0
        ) .+ (1:constraints[t].num_jacobian)
    ) for t = 1:length(constraints)]
end

function hessian_indices(
    constraints::Vector{Dynamics{T}},
...
function state_indices(constraints::Vector{Dynamics{T}}) where T
    [[collect(
        (t > 1 ?
            sum(
                [constraints[s].num_state + constraints[s].num_action
                    for s = 1:(t-1)]
            ) : 0
        ) .+ (1:constraints[t].num_state)
    ) for t = 1:length(constraints)]...,
    collect(
        sum(
            [constraints[s].num_state + constraints[s].num_action
                for s = 1:length(constraints)]
        ) .+ (1:constraints[end].num_next_state)
    )]
end
...
function action_indices(constraints::Vector{Dynamics{T}}) where T
    [collect(
        ...
        ) + constraints[t].num_state .+ (1:constraints[t].num_action)
    ) for t = 1:length(constraints)]
end
...
function state_action_indices(constraints::Vector{Dynamics{T}}) where T
    [[collect(
        ...
        ) .+ (1:(constraints[t].num_state + constraints[t].num_action))
    ) for t = 1:length(constraints)]...,
    collect(
        sum(
            [constraints[s].num_state + constraints[s].num_action
        ...
        (t > 1 ?
            sum(
                [constraints[s].num_state + constraints[s].num_action
                 ...
                 constraints[t].num_action +
                 constraints[t].num_next_state))
    ) for t = 1:length(constraints)]
end

function dimensions(
    dynamics::Vector{Dynamics{T}};
    parameters=zeros(Int, length(dynamics) + 1)
) where T
