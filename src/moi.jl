...
function MOI.eval_constraint(nlp::NLPData{T}, violations, variables) where T
    ...
    if !isempty(nlp.indices.stage_constraints)
        constraints!(
            violations,
    ...
    if nlp.general_constraint.num_constraint != 0
        constraints!(
            violations,
...
function MOI.eval_constraint_jacobian(nlp::NLPData{T}, jacobian, variables) where T
    ...
    !isempty(nlp.indices.stage_jacobians) && jacobian!(
        jacobian,
        nlp.indices.stage_jacobians,
        nlp.trajopt.constraints,
        nlp.trajopt.states,
        nlp.trajopt.actions,
        nlp.trajopt.parameters
    )
    nlp.general_constraint.num_constraint != 0 && jacobian!(
        jacobian,
        nlp.indices.general_jacobian,
        nlp.general_constraint,
        variables,
        nlp.parameters
    )
...
function MOI.features_available(nlp::MOI.AbstractNLPEvaluator)
    if nlp.hessian_lagrangian
...
function MOI.hessian_lagrangian_structure(nlp::MOI.AbstractNLPEvaluator)
    return nlp.hessian_lagrangian_sparsity
end
