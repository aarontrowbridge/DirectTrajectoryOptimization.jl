...
function Solver(
    ...
    options=Options(),
    parameters=[[zeros(num_param) for num_param in dimensions(dynamics)[3]]..., zeros(0)]
) where T
...
function initialize_states!(solver::Solver, states)
    for (t, xt) in enumerate(states)
        ...
        for i = 1:n
            MOI.set(
                solver.data.optimizer,
                MOI.VariablePrimalStart(),
                solver.data.variables[solver.nlp.indices.states[t][i]],
                xt[i]
            )
        end
    end
    ...
    for (t, ut) in enumerate(actions)
        ...
        for j = 1:m
            MOI.set(
                solver.data.optimizer,
                MOI.VariablePrimalStart(),
                solver.data.variables[solver.nlp.indices.actions[t][j]],
                ut[j]
            )
        end
    end
